...
<body class="bg-gray-100 text-gray-800 font-sans antialiased">
    ...
    <div class="min-h-screen flex">
        ...
        {{-- ============ SIDEBAR ============ --}}
        <aside id="sidebar" class="bg-blue-950 text-white w-64 min-h-screen fixed lg:sticky top-0 left-0 z-30 transform -translate-x-full lg:translate-x-0 transition-transform duration-300">
            ...
            <nav class="p-4 space-y-1">
                <a href="{{ route('admin.dashboard') }}" class="flex items-center px-4 py-3 rounded-lg transition {{ request()->routeIs('admin.dashboard') ? 'bg-amber-500 text-blue-950 font-semibold' : 'hover:bg-blue-900' }}">
                    <svg class="w-5 h-5 mr-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 12l2-2m0 0l7-7 7 7M5 10v10a1 1 0 001 1h3m10-11l2 2m-2-2v10a1 1 0 01-1 1h-3m-6 0a1 1 0 001-1v-4a1 1 0 011-1h2a1 1 0 011 1v4a1 1 0 001 1m-6 0h6"/></svg>
                    Dashboard
                </a>

                <a href="{{ route('admin.capacitados.index') }}" class="flex items-center px-4 py-3 rounded-lg transition {{ request()->routeIs('admin.capacitados.*') ? 'bg-amber-500 text-blue-950 font-semibold' : 'hover:bg-blue-900' }}">
                    <svg class="w-5 h-5 mr-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 20h5v-2a3 3 0 00-5.356-1.857M17 20H7m10 0v-2c0-.656-.126-1.283-.356-1.857M7 20H2v-2a3 3 0 015.356-1.857M7 20v-2c0-.656.126-1.283.356-1.857m0 0a5.002 5.002 0 019.288 0M15 7a3 3 0 11-6 0 3 3 0 016 0zm6 3a2 2 0 11-4 0 2 2 0 014 0zM7 10a2 2 0 11-4 0 2 2 0 014 0z"/></svg>
                    Capacitados
                </a>

                <a href="{{ route('admin.cursos.index') }}" class="flex items-center px-4 py-3 rounded-lg transition {{ request()->routeIs('admin.cursos.*') ? 'bg-amber-500 text-blue-950 font-semibold' : 'hover:bg-blue-900' }}">
                    <svg class="w-5 h-5 mr-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 6.253v13m0-13C10.832 5.477 9.246 5 7.5 5S4.168 5.477 3 6.253v13C4.168 18.477 5.754 18 7.5 18s3.332.477 4.5 1.253m0-13C13.168 5.477 14.754 5 16.5 5c1.747 0 3.332.477 4.5 1.253v13C19.832 18.477 18.247 18 16.5 18c-1.746 0-3.332.477-4.5 1.253"/></svg>
                    Cursos
                </a>

                <a href="{{ route('admin.categorias.index') }}" class="flex items-center px-4 py-3 rounded-lg transition {{ request()->routeIs('admin.categorias.*') ? 'bg-amber-500 text-blue-950 font-semibold' : 'hover:bg-blue-900' }}">
                ...
                <a href="{{ route('admin.certificados.index') }}" class="flex items-center px-4 py-3 rounded-lg transition {{ request()->routeIs('admin.certificados.*') ? 'bg-amber-500 text-blue-950 font-semibold' : 'hover:bg-blue-900' }}">
                    <svg class="w-5 h-5 mr-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4M7.835 4.697a3.42 3.42 0 001.946-.806 3.42 3.42 0 014.438 0 3.42 3.42 0 001.946.806 3.42 3.42 0 013.138 3.138 3.42 3.42 0 00.806 1.946 3.42 3.42 0 010 4.438 3.42 3.42 0 00-.806 1.946 3.42 3.42 0 01-3.138 3.138 3.42 3.42 0 00-1.946.806 3.42 3.42 0 01-4.438 0 3.42 3.42 0 00-1.946-.806 3.42 3.42 0 01-3.138-3.138 3.42 3.42 0 00-.806-1.946 3.42 3.42 0 010-4.438 3.42 3.42 0 00.806-1.946 3.42 3.42 0 013.138-3.138z"/></svg>
                    Certificados
                </a>

                <a href="{{ route('admin.mensajes.index') }}" class="flex items-center px-4 py-3 rounded-lg transition {{ request()->routeIs('admin.mensajes.*') ? 'bg-amber-500 text-blue-950 font-semibold' : 'hover:bg-blue-900' }}">
                    ...
                    @if($mensajesNuevos > 0)
                        <span class="ml-auto bg-red-500 text-white text-xs px-2 py-0.5 rounded-full">{{ $mensajesNuevos }}</span>
                    @endif
                </a>

                <a href="{{ route('admin.usuarios.index') }}" class="flex items-center px-4 py-3 rounded-lg transition {{ request()->routeIs('admin.usuarios.*') ? 'bg-amber-500 text-blue-950 font-semibold' : 'hover:bg-blue-900' }}">
                ...
                <div class="border-t border-blue-900 my-4"></div>

                <a href="{{ route('home') }}" target="_blank" class="flex items-center px-4 py-3 rounded-lg hover:bg-blue-900 transition">
                ...
                <div class="px-4 sm:px-6 lg:px-8 py-3 flex items-center justify-between">
                    ...
                    <div class="relative">
                        <button id="user-menu-btn" class="flex items-center space-x-2 px-3 py-2 rounded-md hover:bg-gray-100 transition">
                            <div class="w-8 h-8 bg-amber-500 text-blue-950 rounded-full flex items-center justify-center text-sm font-semibold">
